...
ESS2023_Suomi <- ESS2023_Suomi |>
  mutate(across(c(E9, E10, E11), \(x) na_if(x, 4)))
```
...
Muuttujien valinnan jälkeen tulee funktio, jota kaikkiin valittuihin sovelletaan. Tässä jos pitää määritellä funktiolle argumentteja tai muuta (kuten tässä itse laskutoimitus), käytetään lambda-funktiota muodossa `\(x) operaatio`. `x` viittaa muuttujaan, jota vuorollaan operoidaan.
...
```{r}
ESS2023_Suomi <- ESS2023_Suomi |> 
  mutate(across(starts_with("H"), \(x) 7 - x, .names = "{col}_kaannetty"))
```
...
Lopuksi kannattaa määritellä `.default = NA_character_` tai vastaava argumentti, jonka perusteella ne joihin mikään ehto ei pätenyt menevät puuttuvaksi oikein. Ja jos haluaa muuttujan olevan luokiteltu, niin se pitää vielä as_factorilla kertoa.
